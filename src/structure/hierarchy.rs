...
impl Hierarchy {
    ...
    /// assert_eq!( h.lineage(4), vec![2,3,4] );
    /// ```
    pub fn remove_parent(&mut self, child: Id) -> ActionResult<()> {
        if !self.is_child(child) {
    ...
    fn set_child(&mut self, parent: Id, child: Id) -> ActionResult<()> {
        if !self.is_child(child) {
    ...
    fn remove_child(&mut self, parent: Id, child: Id) -> ActionResult<()> {
