...
pub fn move_turn_before(
    state: event::State,
    entity_id_to_move: entity::Id,
    before_entity_id: entity::Id,
) -> event::ActionResult {
    if !state.round.sequence.contains(&entity_id_to_move) {
        return Err("Unable to move a nonexistant turn before another turn.".to_string());
    }
    if !state.round.sequence.contains(&before_entity_id) {
        return Err("Unable to move a turn to before a nonexistant turn.".to_string());
    }
    let entity_id_to_move_index = state.round.sequence.iter().position(|&seq_e_id| seq_e_id == entity_id_to_move).unwrap();
    let before_entity_id_index = state.round.sequence.iter().position(|&seq_e_id| seq_e_id == before_entity_id).unwrap();
    ...
    update_turn_order(state, entity_id_to_move, distance)
}
...
pub fn update_turn_order(
    ...
    let index = round.sequence.iter().position(|&seq_e_id| seq_e_id == entity_id);
    ...
    if new_index >= round.sequence.len() as i8 {
        return Err("Can't move turn that late in the round.".to_string());
    }

    let mut sequence = round.sequence.clone();
    sequence.remove(index);
    sequence.insert(new_index as usize, entity_id);
...
pub fn update_turn_state(state: event::State, entity_id: entity::Id, turn_status: TurnStatus) -> event::ActionResult {
    if !state.round.sequence.contains(&entity_id) {
        return Err("Can not update status of a turn that doesn't exist in the sequence.".to_string());
    }

    let mut turns = state.round.turn_states;
    turns.insert(entity_id, turn_status);
...
pub fn activate_delayed_turn(
    ...
    if ! state.round.sequence.contains( &triggering_entity_id ) {
        return Err("Can not activate delayed turn by nonexistant triggering turn.".to_string() );
    }

    let turn_status = state.round.turn_states.get(&entity_id);

    if let Some(TurnStatus::Held(held_round_count)) = turn_status {

        let delayed_turn_position = state.round.sequence.iter().position(|&seq_e_id| seq_e_id == entity_id).unwrap();
        let triggering_turn_position = state.round.sequence.iter().position(|&seq_e_id| seq_e_id == triggering_entity_id).unwrap();
        ...
        if held_from_current_round && delayed_turn_position < triggering_turn_position ||
            ...
            return update_turn_state(state, entity_id, TurnStatus::Active)
        } else {
...
pub fn order_turns_by_initiative(state: event::State) -> event::ActionResult {
    let mut round = state.round;

    if round.sequence.len() < 2 {
    ...
    let mut sequence = round.initiatives
        .iter()
        .map(|(entity_id, initiative)| (*entity_id, *initiative))
        .collect::<Vec<(entity::Id, Initiative)>>();

    sequence.sort_by(|(_, a), (_, b)| a.partial_cmp(b).unwrap());

    let sequence = sequence.iter()
...
) -> event::ActionResult
{
    ...
    if entity::by_id(&state, entity_id).is_none() {
        return Err("Unable to add entity turn. Entity does not exist.".to_string());
    }


    let mut round = state.round;
    round.initiatives.insert(entity_id, initiative);
    round.turn_states.insert(entity_id, TurnStatus::Available);

    // @todo , consider adding at specific point/index. If setup, auto order?
    round.sequence.push(entity_id);
...
pub fn remove_turn(state: event::State, entity_id: entity::Id) -> event::ActionResult {
    if !state.round.sequence.contains(&entity_id) {
    ...
    round.sequence = round.sequence
        .into_iter()
        .filter(|e_id| e_id != &entity_id)
        .collect();
    round.turn_states.remove(&entity_id);
    round.initiatives.remove(&entity_id);
...
pub fn transition_next_round_turn_states(turn_states: TurnStates) -> TurnStates {
    let mut turn_states = turn_states;
    turn_states
        .into_iter()
        .map(
...
pub fn next_round(state: event::State) -> event::ActionResult {
    if state.round.turn_states.is_empty() {
        return Err("Can not proceed to next round. There are no turns in the sequence.".to_string());
    }
    for (_, turn_state) in state.round.turn_states.iter() {
        match *turn_state {
    ...
    Ok(event::State {
        round: State {
            count: state.round.count + 1,
            turn_states: transition_next_round_turn_states(state.round.turn_states),
            ..state.round
        },
        ..state
    })
